...
const Facture = props => (
  ...
  <tr>
    ...
    <td>
      <a href="/facturelist" onClick={(e) => { props.deleteThisGoal(e,props.devis._id) }}>
...
export default class FactureList extends Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:3001/Facture/')
      .then(response => {
  ...
  deleteDevis(id) {
    axios.delete('http://localhost:3001/Facture/'+id)
  ...
  devisList() {
    return this.state.devisl.map(currentdevis=> {
      return <Facture devis={currentdevis} deleteThisGoal={this.deleteThisGoal} key={currentdevis._id}/>;
    })
  }
